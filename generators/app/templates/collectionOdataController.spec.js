describe('<%= controllerName %> test', function() {
	...
	it('Открываем выполняется запрос', function() {
		$httpBackend.expect('GET', '/odata<%= comeBackUrl %>').respond([]);
	...
	it('Открываем выполняется запрос - ошибка', function() {
		var name, promise;
		$httpBackend.expect('GET', '/odata<%= comeBackUrl %>').respond(500, '');
	...
	it('Удаление - ошибка', function() {
		...
		$httpBackend.expect('DELETE', '/odata<%= comeBackUrl %>(1)').respond(500, '');
	...
	it('Удаление', function() {
		...
		$httpBackend.when('GET', '/odata<%= comeBackUrl %>').respond([]);
		var controller = $controller(controllerName, {
			$scope: scope,
			modalService: modalService,
			notificationService
		...
		$httpBackend.expect('DELETE', '/odata<%= comeBackUrl %>(1)').respond({});
		controller.delete({<%= id %>: 1});
		$httpBackend.flush();
		expect(modalService.confirm).toHaveBeenCalled();
		expect(notificationService.notice.calls.argsFor(0)[0]).toBe('delete');
